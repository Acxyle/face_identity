...
class Trainer:
    ...
    def training_parser(add_help=True):
        ...
        #parser.add_argument("--data-path", default="/home/acxyle-workstation/Dataset/CelebA_fold_0", type=str, help="dataset path")
        ...
        parser.add_argument("--lr", default=1e-1, type=float, help="initial learning rate")     # (depends on dataset) 1e-2 for VGG and SpikingResnet > 50
        ...
        parser.add_argument("--ra-reps", default=4, type=int, help="number of repetitions for Repeated Augmentation (default: 4)")

        # Prototype models only
        parser.add_argument("--prototype", dest="prototype", help="Use prototype model builders instead those from main area", action="store_true")
        ...
        parser.add_argument("--local-rank", type=int, help="args for DDP, which should not be set by user")
        parser.add_argument("--disable-torch-deterministic", action="store_true", help="not set 'torch.use_deterministic_algorithms(True)', which can avoid the error raised by some functions that do not have a deterministic implementation")
    ...
    def set_lr_scheduler(self, args, optimizer):
        ...
        """
            this function concatenate 2 lr_scheduler: main_lr_scheduler and warmup_lr_scheduler
    ...
    def main(self, args):
        ...
        set_deterministic(args.seed, args.disable_torch_deterministic)
