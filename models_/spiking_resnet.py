...
"""
[Warning] this code has been modified for feature ectraction
"""
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.sn2 = spiking_neuron(**deepcopy(kwargs))
        self.conv3 = conv1x1(width, planes * self.expansion)     # [notice] channel expansion
...
class SpikingResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x_list = self.layer1(x04_)     # input is tensor, output is list
        x_list = self.layer2(x_list)     # input is list, output is list
        x_list = self.layer3(x_list)
        x_list = self.layer4(x_list)

        x05_ = self.avgpool(x_list[-1])
        
        if self.avgpool.step_mode == 's':     # [notice] 注意不同代码间 avgpool 和 flatten 的使用
