...
class SpikingVGG(nn.Module):
    ...
    def forward(self, x):     # how about feed a list as input?
        ...
        x01_ = self.avgpool(x_list[-1])
        
        if self.avgpool.step_mode == 's':
            x01_ = torch.flatten(x01_, 1)
        elif self.avgpool.step_mode == 'm':
            x01_ = torch.flatten(x01_, 2)
            
        x_list2 = self.classifier([x01_])
        #x02_ = nn.Softmax(dim=-1)(x_list2[-1])
        
        feature = [*x_list[1:], x01_, *x_list2[1:]]
    ...
    def make_layers(cfg, batch_norm=False, norm_layer=None, neuron: callable = None, **kwargs):
        ...
        for v in cfg:
            if v == 'M':
                ...
                if batch_norm:
                    layers += [conv2d, norm_layer(v), neuron(**deepcopy(kwargs))]       # the basic structure of layer with BN
                else:
...
if __name__ =='__main__':
    model = spiking_vgg16_bn(spiking_neuron=neuron.IFNode, surrogate_function=surrogate.ATan(), num_classes=50, detach_reset=True)
