...
save_path = idx_root + '/Freq'
if not os.path.exists(save_path):
    os.makedirs(save_path)

def obtrain_encode_class_dict(verbose=False):
    """
    [action required] 
    
    1. this version only record the encode information between layer and identity 
...
#     num_per_class = 10
#     
#     for feature_path in tqdm(feature_list):     # for each layer
...
#                     print('\nlayer: {}, {} neurons (SI: {}, MI: {}) pass the threshold (all neuron: {}, selective neuron: {}).'.format(
#                         layer, len(list(SI_idx.keys()))+len(list(MI_idx.keys())), len(list(SI_idx.keys())), len(list(MI_idx.keys())), feature.shape[1], col))
#                     SIMI_dict.update({layer: [{'neuron_amount': feature.shape[1]},{'selective_neuron_amount': col},{'SI_idx': SI_idx}, {'MI_idx': MI_idx}]})
#                     
#     with open(os.path.join(save_path, 'ID_neuron_encode_class_dict.pkl'), 'wb') as f: # save the relationship betwwen layer (include SI and MI) and encoded classes
#         pickle.dump(SIMI_dict, f)
# =============================================================================

def load_encode_class_dict():
    with open(os.path.join(save_path, 'ID_neuron_encode_class_dict.pkl'), 'rb') as f:
...
def draw_encode_frequency():
    ...
    with open(os.path.join(save_path, 'encode_class_dict.pkl'), 'rb') as f:
        encode_class_dict = pickle.load(f)
    
# =============================================================================
#     temp_dict = {}
#     
#     for k, v in encode_class_dict.items():  # for each layer
#         encode_class = []
    ...
    for idx, layer in enumerate(layers):    # for each layer
        ...
        for item in encode_class_list:  # for each class
            if (item in freq):  # update the dict or add new k-v pair
                ...
                freq[item] = 1
        freq = {k: v / dim_list[idx] for k, v in freq.items()}    # convert v from abs avlue to ratio
...
if __name__ == "__main__":
    #obtrain_encode_class_dict()
    #draw_encode_frequency()
    draw_encode_frequency_for_each_layer()
    draw_merged_encode_frequency_for_each_layer()
