...
[Key note]
    1.based on the matlab code, convert and rewrite them to python version
    
[log]:
    -
    [notice] all function with variable 'inds' and writing style like 'AaaBbbCcc' is not modified according to my writing style
...
class Selectiviy_Analysis_Correlation_Monkey():
    ...
    def __init__(self, 
                 # generated local
                 #corr_root='/media/acxyle/Data/ChromeDownload/Identity_VGG_Feature_Original/neuron_idx/Correlation/', 
                 # obtained from Dr Cao
                 #corr_root='/home/acxyle/Downloads/osfstorage-archive-supp/FeatureCM/',
                 # spiking model
                 corr_root = '/media/acxyle/Data/ChromeDownload/Identity_SEWResnet50_LIF_CelebA2622_Neuron/Correlation/',
                 
                 bio_neuron_root='/home/acxyle/Downloads/osfstorage-archive-supp/Res/IT_FR_CA_Range70-180.mat',
        ...
        # [notice] in current experiment of monkey, only use 'All neuron' type. 
        # because IT neurons are channel neurons, each channel of 63 channels contains multiple neurons. 
        # So that's inproper to divide the 63 'neurons' into (non-)identity-selective neuron groups
    ...
    def monkey_neuron_analysis(self):
        ...
        uDMN, uDMNPerm, uDMN_T, uDMN_TPerm = self.monkey_neuron_spikes_process()
    ...
    def representational_similarity_analysis(self, uDMN, uDMNPerm, uDMN_T, uDMN_TPerm, nPerm=1000):
        ...
        for ll in tqdm(range(nLayers)):
            ...
            pFN.append((rFNIDPerm_seg > r_).mean())
        ...
        for ll in range(nLayers):
            ...
            sig_T[ll] = np.flatnonzero(pFN_T[ll, :] < 0.05 / 26)
    ...
    def plot_temporal_correlation(self, rFNID_T, sig_T):     # variable: sig_T can be 'sig_T' or 'sig_T_FDR' (former is better)
    ...
    # [comment] looks like the figure can be directly plotted but the function aims to generate several values? 
...
class Selectiviy_Analysis_Correlation_Human():
    '''
    [tasks]
    devide computation and plot
    '''
    def __init__(self,
                 # baseline
                 #corr_root = 'FeatureCM/'
                 # local trained correlation
                 corr_root='/media/acxyle/Data/ChromeDownload/Identity_SpikingVGG16bn_LIF_CelebA9326_Neuron/Correlation/',
                 
                 root_process='/home/acxyle/Downloads/osfstorage-archive-supp/',
                 root_data='/home/acxyle/Downloads/osfstorage-archive/',
                 layers=None):
        # in fact, this variables are all about human neuron process befor RSA with ANN neuron
    ...
    def human_neuron_RSA_analysis(self, used_ID='top50'):
        ...
        # --- generate biological neuron features ONLY based on selected identities
        DM_IDN, DM_IDN_Perm, DM_IDN_T, DM_IDN_T_Perm = self.human_neuron_spikes_process(sorted_ID)
        
        # --- generate artificial unit features and compare with neurons
        #SelMet = 'vKeep'
        #rFNID, rFNID_T, rPermID, pFNID_FDR, pFNID_T_FDR, sigFN_T, sigFDR_T = self.human_neuron_RSA_sub_ID(DM_IDN, DM_IDN_Perm, DM_IDN_T, DM_IDN_T_Perm, sorted_ID, SelMet)
        ...
        print('[Codinfo] Calculting vKeep(all_neuron)...')
        self.human_neuron_RSA_analysis_SelMet(DM_IDN, DM_IDN_Perm, DM_IDN_T, DM_IDN_T_Perm, sorted_ID, used_ID, SelMet='vKeep')
        print('[Codinfo] Calculting IDNeuron...')
        self.human_neuron_RSA_analysis_SelMet(DM_IDN, DM_IDN_Perm, DM_IDN_T, DM_IDN_T_Perm, sorted_ID, used_ID, SelMet='IDNeuron')
        print('[Codinfo] Calculting nonIDNeuron...')
        self.human_neuron_RSA_analysis_SelMet(DM_IDN, DM_IDN_Perm, DM_IDN_T, DM_IDN_T_Perm, sorted_ID, used_ID, SelMet='nonIDNeuron')
    ...
    def select_sub_identities(self, neuron_dict, subSelectID = '_10_IDNeuron', used_ID='top10'):
        ...
        # ----- select used_ID
        if 'top' in used_ID:
            sorted_ID = [i[0] for i in self.sub_ID_selection(codeIDAll, int(used_ID[3:]))]
    ...
    def human_neuron_spikes_process(self, sorted_ID, SelMet='IDNeuron', nPerm=1000):
        ...
        # [warning] no reason for this action, seems the number of selected neurons can vary   
        #CellToAnalyze = CellToAnalyze.reshape(-1)[:490]-1
        CellToAnalyze = CellToAnalyze.reshape(-1)-1  
        
        label = self.Label['label'].reshape(-1)
        
        # cal similarity matrix of FR across neurons
        ...
        # [notice] this section is required to analysis the difference between above section
        IDRes = []
        IDPSTH = []
    ...
    def human_neuron_RSA_sub_ID(self,DM_IDN, DM_IDN_Perm, DM_IDN_T, DM_IDN_T_Perm, sorted_ID, used_ID, SelMet, subSelectID='_10_IDNeuron', nPerm=1000):
        ...
        # [notice] here should have more than 3 types of neuron, what if add the SI and MI section?
        if SelMet == 'IDNeuron':
        ...
        for ll in tqdm(range(len(layers))):
            layer = layers[ll]
            rIDF = DNNID[layer]     # directly call the ANN data
    ...
    def human_neuron_RSA_sub_ID_plot(self, rFNID, rPermID, pFNID_FDR, SelMet, used_ID):
        ...
        plt.xticks(np.arange(len(rFNID)), layers, rotation='vertical')
    ...
    def human_neuron_get_firing_rate(self):
...
if __name__ == "__main__":
    ...
    #FIXME
    # [notice] this section only provide the NAME of layers and NUMBER of neurons
    ...
    layers = [i for i in layers if 'act' in i or 'pool' in i or 'fc' in i]
    
    corr_root = '/media/acxyle/Data/ChromeDownload/Identity_Resnet50_Original_Neuron/Correlation/'
    
    # for monkey experiments
    test = Selectiviy_Analysis_Correlation_Monkey(corr_root=corr_root, layers=layers)
    test.monkey_neuron_analysis()
    
    # for human experiments 
    test = Selectiviy_Analysis_Correlation_Human(corr_root=corr_root, layers=layers)
