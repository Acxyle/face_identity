...
def get_args_parser(fold_idx):
    ...
    parser.add_argument("--model", type=str, default='spiking_vgg16_bn')     # [notice] this will triger different branches
    ...
    parser.add_argument("--output_dir", type=str, default=f'Face Identity SpikingVGG16bn_IF_T4_CelebA2622_fold_{fold_idx}')
    parser.add_argument("--weight_path", type=str, default=f"logs_ft_CelebA50_spikingvgg16bn_IF_T4_CelebA2622_fold_{fold_idx}")
...
if __name__ =="__main__":
    ...
    for fold_idx in [1,2,3,4]:
        ...
        if 'spiking' in args.model.lower():
            spikingjelly_evaluate(model, layers, layers, neurons, args, verbose=False)
        else:
            ANN_evaluate(model, layers, layers, neurons, args, verbose=True)
