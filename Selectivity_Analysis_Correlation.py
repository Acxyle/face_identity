...
class Selectiviy_Analysis_Correlation_Human():
    ...
    def human_neuron_raster_plot(self):
        ...
        for ii in range(len(CellToPlot)):  # for each neuron
            ...
            if session_idces[iSess]['taskInstruction'][0] == 'Loc_Face':
            ...
            id_code = sio.loadmat(os.path.join(self.root_process, session_idces[iSess]['taskInstruction'][0] + '_Image_Code_new.mat'))['id_code'][0]
            FR_ID = []
    ...
    def human_neuron_sort_FR(self, rejectRate=0.15, dataType='FR', taskInstruction='CelebA'):
        """
            [notice] whe nthe first time call a variable from the 1-based MATLAB data, convert it to 0-based python format
        """
        # ----- 1. obtain firing rate (FR) and peri-stimulus histogram (PSTH)
        FR_stats = self.human_neuron_get_firing_rate()
        # -----
        
        session_idx_names = self.get_session_idces()     # <- stores time periods of each trial (each session_idxion)
        
        # Get behavior, use an independent function to replace this later
        beh = []
        Acc = []
    
        # ----- this should not in this code, this is the data preparation
        for session_name in tqdm(session_idx_names, desc='sort beh'):
            
            subname = session_name.split('_')[0]  # 'pXXX'
            
            if 'Sess' in session_name:
                sessnum = session_name.split('_')[-1]
            else:
                sessnum = 'Sess' + session_name[session_name.find('S')+1]
                
            ibeh = self.GetBehavior3(subname, sessnum)
            beh.append(ibeh)  
            
            Acc.append(np.sum(ibeh['vResp'])/len(ibeh['back_id']))     # [notice] looks like some records have quite low accuracy
        # -----
        
        # -----
        vReject = np.array([]).astype(int)
        # === 1st
        for iCell in range(len(FR_stats)):  
            if np.nanmean(FR_stats[iCell]['spike_count_0_2000']) < rejectRate:  
                vReject = np.append(vReject, iCell)
        
        # === 2nd
        neuron_session_idces = self.Spikes['vCell'].reshape(-1)-1     # 1-based (MATLAB) -> 0-based (Python)
        
        # manually "exclude sessions 12(only has 117 trials) and 18(only 1 neuron kept and patients did not pay attention)"
        exCell = np.array([index for index, value in enumerate(neuron_session_idces) if value == 11 or value == 17])
        vReject = np.union1d(vReject, exCell)
 
        vKeep = np.setdiff1d(np.arange(len(FR_stats)), vReject)     # qualified neurons with 2 conditions
        # -----
        
        CelebA_Image_Code = sio.loadmat(os.path.join(self.root_data, 'SingleNeuron/Code/CelebA_Image_Code.mat'))
        AdjustInd = CelebA_Image_Code['AdjustInd'].reshape(-1).astype(np.int16) -1     
        
        meanFR = np.full((len(FR_stats), 500), np.nan)
        
        for iCell in tqdm(range(len(FR_stats))):     # for each neuron
            
            iSess = neuron_session_idces[iCell]   # get session idx
        
            Code = beh[iSess]['code'].copy()   # delete back_id in img_list
            back_id = beh[iSess]['back_id'].copy()

            Code = np.delete(Code, back_id)
            
            if dataType == 'Base':
                tmpFR = FR_stats[iCell]['spike_count_250_500'].copy()
            elif dataType == 'FR':
                tmpFR = FR_stats[iCell]['spike_count'].copy()
            
            tmpFR = np.delete(tmpFR, back_id)   # remove back_id in neuron response
            
            I = np.argsort(Code)     # the index of 'Code' in ascending order
            A = Code[I]     # sort the 'Code' in ascending order
            sortFR = tmpFR[I]     # re-arrange the tmpFR based on the index of 'Code', make the responses align with the image idces
        
            # [notice] in MATLAB code, there has non-CelebA settings
            """
                for those session with code != 500, usually means the images displayed in the experiments are not standard
                set.
            ...
            if taskInstruction == 'CelebA':
                ...
                if iSess < 10:
                    ...
                    if len(Code) != 500:
                        ...
                        newSortFR = np.full(500, np.nan)  
                        
                        for jj in range(500):     # for each image
                            tmpind = np.where(A == (jj+1))[0]     # for most of the cases, 1 record for 1 img
                            if len(tmpind) != 0:
                                newSortFR[jj] = np.mean(sortFR[tmpind])   # take average 
                                
                        for _ in range(500):
                            if AdjustInd[_] >= 0:
                                adjustFR[_] = newSortFR[AdjustInd[_]]
                    else:
                        
                        for _ in range(500):
                            if AdjustInd[_] >= 0:
                                adjustFR[_] = sortFR[AdjustInd[_]]
                    
                    meanFR[iCell, :] = adjustFR  # Note: Python uses 0-based indexing
                    ...
                    if len(Code) != 500:
                        ...
                        for jj in range(500):
                            tmpind = np.where(A == (jj+1))[0]
                            if len(tmpind) != 0:
                                newSortFR[jj] = np.mean(sortFR[tmpind])
                                
                        meanFR[iCell, :] = newSortFR
                ...
                raise RuntimeError('[Coderror] taskInstruction not applicable')
                
                # [notice] legacy from MATLAB code, not in use for current task
                #tmpFR = np.array([])
                #for jj in range(1, imgNum+1):
                #    tmpInd = np.where(A ==jj)
                #    if not tmpInd:
                #        tmpFR = np.concatenate((tmpFR, sortFR[tmpInd]))
                #meanFR[iCell-1,:] = tmpFR;
            
        if taskInstruction == 'CelebA':
            
            CelebA_Image_Code_new = sio.loadmat(os.path.join(self.root_data, 'SingleNeuron/Code/CelebA_Image_Code_new.mat'))
            
            id_code = CelebA_Image_Code_new['id_code'].reshape(-1)
            
            # ----- 1.
            # face 121 is a mis-identified photo of Identity 6, replace the FR to average FR of other 9 faces
            meanFR[:,120] = np.nanmean(meanFR[:, id_code[5].reshape(-1)[:-1]-1], 1)
            
            # ----- 2.
            problemID = [17,39]
            for pp in problemID:
                tmpInd = id_code[pp].reshape(-1)[-1] - 1
                meanFR[:381, tmpInd] = np.nanmean(meanFR[:381,id_code[pp].reshape(-1)[:-1]-1], 1)
            
        return meanFR
        
    def GetBehavior3(self, subname, sessnum):     # this function should direct to behavior data
        """
            ...
            Behavior data of each session was stored in the .mat file. 
                ...
                ‘vResp’ - If the patient responded (i.e., pressed the space bar) in the one-back trial, the value is 1 for 
            this trial in the variable ; otherwise, the value is 0. 
                ‘RT’ - response time.
        
        """
        
        # [notice] rebuild the path
        behDir = os.path.join(self.root_data, 'behaviorData/Archive/')
        
        path = os.path.join(behDir, subname, self.data_set, sessnum)
        
        # beh['iT'].dtype.names: ('TRIAL_START', 'IMAGE_ONSET_try', 'IMAGE_ONSET', 'IMAGE_OFFSET', 'RESP_MSG', 'timeStart', 'ITI', 'ITI_START', 'TRIAL_END', 'RESPONDED')
        
        # ----- data preparation1
        beh = data_preparation(path)
        # -----

        # -----
        if len(beh['iT']['TRIAL_START']) < len(beh['code']):     # no such case in current experiments
            raise RuntimeError(f'Error Detected: {path}')    
            # ----- legacy from MATLAB code
            #beh['code'] = beh['code'][:len_iT]
            #beh['back_id'] = beh['back_id'][np.where(beh['back_id']<=(len_iT+1))[0]]
            #beh['vResp'] = beh['vResp'][:len_iT]
            #beh['RT'] = beh['RT'][:len_iT]
            #beh['vCorr'] = beh['vCorr'][:len_iT]
    ...
    def human_neuron_get_firing_rate(self, time_window=250, time_step=50):  
        ...
        # <- raw timestamps were stored by .mat format from OSF database
        # data is sorted based on 1-based order of MATLAB
        ...
        if os.path.exists(file_path):
            ...
            for session_idxion in tqdm(range(len(session_idx_names)), desc='Load session_idces'):
                # ↓ 'periods' contains 3 columns: trial indices, timestamps to 500 before stumuli onset, timestamps to 1500 after stimuli onset
                periods = sio.loadmat(os.path.join(session_idxion_dir, session_idx_names[session_idxion]+'.mat'))['periods']     
...
def data_preparation(path):
    ...
    # --- [p9WV Sess2] the only one has 2 records --- [question] why not independently process that?
    ...
    if 'p9WV' in path and 'Sess2' in path:     #target: concatenate 2 records
        
        if inputfiles != ['CelebA_p9WV_26Oct2019163716.mat', 'CelebA_p9WV_26Oct2019164627.mat']:     # file check
            raise RuntimeError('[Coderror] P9WV_Sess2 records incorrect')
        
        # -----
        for filename in inputfiles:
            
            beh_dict = load_initial_experiment_data_and_conversion(os.path.join(path, filename))
            
            # ----
            if filename == 'CelebA_p9WV_26Oct2019163716.mat':     # cut-off
                
                record_length = len(beh_dict['resp_log'])     # 161
                back_id_length = np.where(beh_dict['back_id']<record_length)[0].size     # 16
                
                beh1 = {variable: beh_dict[variable][:record_length] for variable in variables_list}
                beh1['iT'] = {_: beh_dict['iT'][_][:record_length] for _ in beh_dict['iT'].keys()}
                beh1['back_id'] = beh_dict['back_id'][:back_id_length]
                
                beh.update(**beh1)
                
            elif filename == 'CelebA_p9WV_26Oct2019164627.mat':     # add the last run
            
                beh2 = {variable: beh_dict[variable] for variable in variables_list}
                beh2['iT'] = {_: beh_dict['iT'][_] for _ in beh_dict['iT'].keys()}
                beh2['back_id'] = beh_dict['back_id']
                
                beh2['back_id'] = beh_dict['back_id'] + record_length
        # -----
            
        # ----- concatenate 2 records
        for key in [_ for _ in list(beh.keys()) if 'iT' not in _]:
            beh[key] = np.concatenate((beh[key], beh2[key]))
        for key in beh['iT'].keys():
            beh['iT'][key] = np.concatenate((beh['iT'][key], beh2['iT'][key]))
        # -----
            
    # ----- one operation is required to merge those 2 records of [P9WV Sess2]
    else:
        ...
        # -----
        if filename == 'CelebA_p10WV_02Feb2020152353.mat':     # cut-off, P10WV_Sess3
            record_length = len(beh_dict['resp_log'])     # 182
            back_id_length = np.where(beh_dict['back_id']<record_length)[0].size     # 18
            
            beh = {variable: beh_dict[variable][:179] for variable in variables_list}
            beh['iT'] = {_: beh_dict['iT'][_][:179] for _ in beh_dict['iT'].keys()}
            beh['back_id'] = beh_dict['back_id'][:18]
            
        elif filename == 'CelebA_p11WV_25Feb2020095049.mat':     # cut-off
            beh = {variable: beh_dict[variable][54:] for variable in variables_list}
            beh['iT'] = {_: beh_dict['iT'][_][54:] for _ in beh_dict['iT'].keys()}
            beh['back_id'] = beh_dict['back_id'][6:] - 54
        # -----
        
        # ----- local designed operation for the 2 special records
        elif filename == 'CelebA_p7WV_20Sep2019101736.mat':
            beh = {variable: beh_dict[variable][2:] for variable in variables_list}
            beh['iT'] = {_: beh_dict['iT'][_][2:] for _ in beh_dict['iT'].keys()}
            beh['back_id'] = beh_dict['back_id'] - 2
            
        elif filename == 'CelebA_p9WV_27Oct2019113332.mat':
            beh = {variable: beh_dict[variable][1:] for variable in variables_list}
            beh['iT'] = {_: beh_dict['iT'][_][1:] for _ in beh_dict['iT'].keys()}
            beh['back_id'] = beh_dict['back_id'] - 1
        # -----
        
        # -----
        else:
            beh = {variable: beh_dict[variable] for variable in variables_list}
            beh['iT'] = {_: beh_dict['iT'][_] for _ in beh_dict['iT'].keys()}
            beh['back_id'] = beh_dict['back_id']
        # -----

    # adjustBehaviorAccordingToTTL()  
    # <- can not find such file and looks like the function is refining 2 inhomogenous seessions to normal shape
    # also use TTL to refine the exact timing.
    
    # ----- seems not in use?
    beh['isEyeTrack'] = beh_dict['isEyeTrack'].item()
    beh['stimWindowSize'] = beh_dict['stimWindowSize']
    beh['windowRect'] = beh_dict['windowRect']
    beh['T'] = beh_dict['T']
    # -----
    
    return beh

def load_initial_experiment_data_and_conversion(path):
    """
        load and convert data from MATLAB structure to Python dict
    """
...
def compare_beh_and_behm(beh):
    ...
    beh_m = [{__: beh_m[__][_].reshape(-1) for __ in beh[0].keys()} for _ in range(40)]
    ...
    for i in range(40):
        for key in beh[i].keys():
            if isinstance(beh[i][key], np.ndarray):
                if not np.all(beh[i][key][~np.isnan(beh[i][key])] == beh_m[i][key][~np.isnan(beh_m[i][key])]):
                    print(i, key, np.all(beh[i][key][~np.isnan(beh[i][key])] == beh_m[i][key][~np.isnan(beh_m[i][key])]))
            if isinstance(beh[i][key], dict):
                if 'iT' in key:
                    for key_ in beh[i][key].keys():
                        if not np.all( beh[i][key][key_][~np.isnan(beh[i][key][key_])] == beh_m[i][key][key_][~np.isnan(beh_m[i][key][key_])] ):
                            print(i, key, key_, np.all( beh[i][key][key_][~np.isnan(beh[i][key][key_])] == beh_m[i][key][key_][~np.isnan(beh_m[i][key][key_])] ))
                elif 'T' in key:
                    for key_ in beh[i][key].keys():
                        if not beh[i][key][key_]==beh_m[i][key][key_]:
                            print(i, key, key_, beh[i][key][key_]==beh_m[i][key][key_])
